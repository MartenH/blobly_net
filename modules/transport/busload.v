...
//
// THE RULE IS BITS ON THE WIRE, and the bit count is the WORST case: bit stuffing adds one bit
// per five equal bits, so a frame is between its bare length and ~1.2× it, and no receiver
// can tell which without the raw bit stream. Tools disagree on this choice (PCAN-View reads
...
// the CRC delimiter (the stuffable field is 34 + 8n / 54 + 8n bits: one stuff bit per 5).
//
...
pub fn frame_bits(f CanFrame, nominal int, data int) f64 {
	...
	if !f.fd {
		...
		return f64(fixed + 8 * n + stuffable / 5)
	}
