...
// no_can: the empty per-channel shell-frame map — CAN targets fall to or_defaults()
const no_can = map[int]telem.ShellFrames{}

// can0_ids: the shell frames map for a single CAN channel at index 0
fn can0_ids() map[int]telem.ShellFrames {
	return {
		0: shell_ids()
	}
}
...
fn test_none() {
	assert shell_targets([], [], telem.SomeipIdent{}, 0, no_can).len == 0
	// a manifest-less channel derives nothing
	assert shell_targets([Channel{
		name: 'A'
	}], [true], telem.SomeipIdent{}, 0, no_can).len == 0
}

fn test_someip_only() {
	ts := shell_targets([someip_chan('board', '10.0.0.5')], [true], eth_ident(), 0x800,
		no_can)
	assert ts.len == 1
	assert ts[0].eth
	assert ts[0].ci == 0
...
fn test_someip_needs_identity() {
	...
	assert shell_targets([someip_chan('board', '10.0.0.5')], [true], eth_ident(), 0, no_can).len == 0
	assert shell_targets([someip_chan('board', '10.0.0.5')], [true], telem.SomeipIdent{},
		0x800, no_can).len == 0
}
...
fn test_someip_dup_excluded() {
	...
	ts := shell_targets([someip_chan('b1', '10.0.0.5'), someip_chan('b2', '10.0.0.6')], [
		true,
		true,
	], eth_ident(), 0x800, no_can)
	assert ts.len == 1
	assert ts[0].ci == 0
}

fn test_can_only() {
	ts := shell_targets([can_chan('CAN')], [true], telem.SomeipIdent{}, 0, can0_ids())
	assert ts.len == 1
	assert !ts[0].eth
	assert ts[0].ci == 0
	assert ts[0].iface == 'vcan0'
	assert ts[0].label == 'CAN · CAN 0x6f0'
	assert ts[0].sh.input == 0x6f0 // the entry carries its own ids for the worker
}
...
fn test_can_defaults_undeclared_frames() {
	...
	ts := shell_targets([can_chan('CAN')], [true], telem.SomeipIdent{}, 0, no_can)
	assert ts.len == 1
	assert ts[0].label == 'CAN · CAN 0x7f0'
	assert ts[0].sh.input == 0x7f0
}

fn test_can_per_target_ids() {
	// two CAN channels with DIFFERENT manifests each route to their OWN command
	// id — a single global id would send channel 0's command on channel 1's bus
	c0 := can_chan('A')
	c1 := can_chan('B')
	frames := {
		0: telem.ShellFrames{
	...
	ts := shell_targets([c0, c1], [true, true], telem.SomeipIdent{}, 0, frames)
	assert ts.len == 2
	assert ts[0].label == 'A · CAN 0x6f0' && ts[0].sh.input == 0x6f0
	assert ts[1].label == 'B · CAN 0x730' && ts[1].sh.input == 0x730
}
...
fn test_runtime_enabled_wins() {
	...
	assert shell_targets([can_chan('A')], [false], telem.SomeipIdent{}, 0, can0_ids()).len == 0
	mut cfg_off := can_chan('B')
	cfg_off.enabled = false
	assert shell_targets([cfg_off], [true], telem.SomeipIdent{}, 0, can0_ids()).len == 1
}
...
fn test_standalone_eth() {
	...
	ts := shell_targets([can_chan('bench')], [true], eth_ident(), 0x800, can0_ids())
	assert ts.len == 2
