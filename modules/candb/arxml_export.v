...
// provenance comment and the E2E attributes as extras the writer places itself.
	...
	for m in msgs {
		...
		if s := f.secoc {
			if s.byte_aligned {
				...
				b << '# SecOC (data_id 0x${s.data_id:X}): a ${s.freshness_tx_len}-bit freshness puts the MAC at bit ${s.mac_bit}, which byte positions cannot express'
			}
		}
	}
	return b.join('\n') + '\n'
