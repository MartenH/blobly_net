...
fn test_claims_oldest_first() {
	...
	r.note(1, 'vcan0', f, 0)
	r.note(2, 'vcan0', f, 5)
	a := r.claim('vcan0', f, 6) or {
		assert false, '${err}'
		return
	}
	b := r.claim('vcan0', f, 7) or {
		assert false, '${err}'
		return
	}
...
fn test_a_different_payload_is_not_our_echo() {
	...
	if _ := r.claim('vcan0', frame(0x120, [u8(1), 2, 4]), 1) {
...
fn test_an_expired_emission_cannot_be_claimed() {
	...
	f := frame(0x120, [u8(1)])
	r.note(1, 'vcan0', f, 0)
	if _ := r.claim('vcan0', f, default_window_ms + 1) {
...
fn test_the_ring_is_bounded() {
	...
	if _ := r.claim('vcan0', frame(0x100, [u8(0)]), 1) {
		assert false, 'a dropped record still claimed a frame'
	}
	seq := r.claim('vcan0', frame(0x109, [u8(9)]), 1) or {
