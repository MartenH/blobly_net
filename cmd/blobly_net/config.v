...
fn (mut app App) set_adapter(i int, a string) {
	...
	if a == 'doip' || a == 'someip' {
		...
		// AND J1939, with the rest of what stopped applying: it is a CAN protocol, the editor
		// hides the tick on an Ethernet row, and left set it was a hidden `j1939: true` that
		// Save persisted and that came back the day the row was converted to CAN again (codex).
