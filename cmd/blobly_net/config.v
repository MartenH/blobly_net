...
fn (mut app App) poll_shortcuts() {
	// Not while the file browser is up: an unsaved project's first Ctrl+S opens Save As, and a
	// second one would open it AGAIN — a fresh, empty name field over the one being typed
	// (codex round 3 on #250). The picker's own Save is the way out of it.
	if app.fb_open {
		return
	}
	if vgui.key_ctrl_only() && vgui.key_pressed(`s`) {
