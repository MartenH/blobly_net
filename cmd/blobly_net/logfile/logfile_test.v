...
// THE NAME CARRIES THE TIME IN UTC, sortably and without a colon (Windows cannot have one in
// a name) — UTC because rotation sorts by name and a local clock steps back once a year.
fn test_the_file_name_is_utc_sortable_and_has_no_colon() {
	// A UTC instant: local_to_utc of a time already in UTC is itself.
	t := time.new(time.Time{ year: 2026, month: 8, day: 29, hour: 16, minute: 23, second: 43, is_local: false })
	name := file_name(t, 4711)
	assert name.ends_with('Z-4711.log')
	assert name.starts_with('blobly_net-2026-08-29T')
	assert !name.contains(':')
	assert is_session_log(name)
	...
	assert !is_session_log('notes-2026-08-29T16-23-43Z-1.log')
	assert !is_session_log('blobly_net-2026-08-29T16-23-43-4711.log'), 'the local-time shape is not ours to rotate'
	assert !is_session_log('blobly_net-2026-08-29T16-23-43Z-abc.log')
}
...
fn test_rotation_removes_the_oldest_of_ours_only() {
	names := ['blobly_net-2026-08-29T16-23-43Z-10.log', 'blobly_net-2026-08-27T09-00-00Z-9.log',
		'readme.txt', 'blobly_net-2026-08-28T12-00-00Z-8.log', 'blobly_net-2026-08-26T08-00-00Z-7.log']
	gone := to_delete(names, 2)
	assert gone == ['blobly_net-2026-08-26T08-00-00Z-7.log', 'blobly_net-2026-08-27T09-00-00Z-9.log']
...
fn test_the_header_states_version_os_project_and_args() {
	h := header(Facts{
		...
		log_path: 'C:/Users/x/AppData/Local/blobly_net/logs/blobly_net-2026-08-29T14-23-43Z-4711.log'
	})
	lines := h.split_into_lines()
	assert lines.len == 5
	assert lines[0] == '# blobly_net 2026.08.01'
	assert lines[1].starts_with('# started 2026-08-29 16:23:43 local')
	assert lines[1].ends_with('blobly_net-2026-08-29T14-23-43Z-4711.log')
