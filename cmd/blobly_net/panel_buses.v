...
fn draw_buses(mut app App, chans []Chan) {
	...
	for k in order {
		...
		for i in idxs {
			...
			if why := transport.wire_silence_fault(c.iface) {
				vgui.same_line()
				vgui.text_colored(u8(240), u8(150), u8(60), 'NOT SILENT')
				vgui.set_item_tooltip('${c.iface}: ${why}. This wire is marked listen-only, but the transceiver is still acknowledging every frame it sees — on a bus with one other node that is the difference between its frames succeeding and it going error-passive. Retried on every receive.')
			}
