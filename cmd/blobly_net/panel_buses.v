		...
		for i in idxs {
			...
			if c.running && !c.doip && c.load_hist.len >= 1 {
				if c.load_hist.len >= 2 {
					...
					vgui.set_item_tooltip('bus load, last ${c.load_hist.len} s — bit-times of DECODED frames on ${c.iface} over ${if c.bitrate > 0 { c.bitrate } else { project.default_bitrate }} bit/s, worst-case stuffing. Error frames, retransmissions and overrun drops are not in it — see the fault ladder')
				}
				vgui.same_line()
