	...
	if !ro && vgui.input_int('id (dec)', &idv) {
		...
		if !id_taken {
			...
			for wi, w in app.watch {
				...
				if edit_wires.any(w.rewritten_by(old_id, wext0, it, j1939.pgn(old_id))) {
	...
	if !ro && next != msg.ext {
		...
		if clash {
			...
			for wi, w in app.watch {
				...
				if app.wires_of_db(di).any(w.rewritten_by(old_id2, old_ext2, it, j1939.pgn(old_id2))) {
					app.watch[wi] = Watch{
						...w
						id:  nid
						ext: next
	...
	if !ro && vgui.input_text('name##dbcs', mut app.dbc_ed.sname_buf) {
		...
		if dbc_ident_ok(nv) && !name_taken {
			...
			for wi, w in app.watch {
				...
				if w.sig == old_sig && app.wires_of_db(di).any(w.rewritten_by(wid, wext, it, j1939.pgn(wid))) {
					app.watch[wi] = Watch{
						...w
						sig: nv
					}
