...
fn main() {
	...
	entries := if stream {
		...
		println('stream: forced ${s.forced}, evicted ${s.evicted}, refused ${s.refused}, unresolved ${s.unresolved}, out of order ${s.out_of_order}, read-ahead high-water ${s.max_queued} rows / ${s.max_skew_s * 1000.0:.1f} ms, writer disorder ${s.max_disorder_s * 1000.0:.1f} ms')
