...
| `transport` (load) | **Bus load** (`busload.v`): bit-times a frame occupies on the wire — classic 47/67 + 8n with worst-case stuffing stated as the choice ((len-1)/4, the textbook 135 bits for a standard 8-byte frame), a remote frame at its true empty length, CAN-FD with its data phase scaled by the rate switch — over the wire's nominal rate. The Buses row shows the last 60 s as a strip and this second as a number; the toolbar carries the busiest running wire. Ours and theirs count alike (a frame on the wire is load whoever sent it) — ours once the driver ACCEPTED the frame, so a refusal has nothing to refund, and onto the wire's RUNNING row, so an alias row's sends are the wire's load and not a skipped row's, and carried to the successor when the reader moves like health and cadence are; a send accepted while the reader is still spawning lands on that row rather than nowhere, and a row nobody reads rolls no history — its strip resets rather than showing a gap as idle; a spawning row closes nothing and rebases, so a slow open is not idle time; a handoff carries the outgoing reader's partial interval — bits and time — into the successor's first sample, which closes once a second has been OBSERVED and takes one column per second it covered, so the strip stays about sixty seconds wide. That rule is `cmd/blobly_net/loadrule`, tested. Rates are ONE answer per wire (`wire_rates_locked`: the first enabled row's), not the reader's row's, resolved once per run and kept so a mid-run alias toggle cannot re-price an interval. **Of DECODED frames only**, and labelled so: an error frame, a retransmission and an overrun drop occupied the wire and arrive as health or as a count, never with a length, so in an error storm the gauge under-reads and the fault ladder beside it is what says so. **The identity predicates answer in place** (`lead_space`, `prefix_fold`, `trimmed`; #300): `wire_key`, `destination_key`, `vendor_iface` and `echoes_own_sends` are asked ~150 times per replayed frame, and spelled with `trim_space()`/`to_lower()` each answer was a throwaway string — `iface_prefix_test.v` pins them to the old spellings. And the inproc fan-out pushes with `try_push` under the registry's read lock — non-blocking, so it cannot wait while holding it, and it builds no target list and runs no `select` per subscriber |
| `candb` | DBC parse/decode/encode + canonical writer (`dbc_write.v`). `Message.j1939` carries `BA_ "VFrameFormat" ... J1939PG`, resolved through the FILE'S OWN enum (`BA_DEF_ BO_ "VFrameFormat" ENUM ...`, read after the whole file since a DBC does not promise record order — Vector's index 3 is a fallback for the malformed case of a numeric value with no definition, not an assumption about a well-formed one) — either spelling (the enum INDEX a BA_ record normally holds, or the quoted choice), and the file-wide `BA_DEF_DEF_` default counts as a declaration a per-message record then overrides. Read back for the one consumer that must not GUESS a bus is J1939 (#289), and WRITTEN back by `dbc_write` so an editor Save does not silently drop it — suppressed there when the caller supplies its own `VFrameFormat` extra, as `export_dbc` does with the richer one that distinguishes the CAN-FD entries. **And AUTOSAR ARXML** (#272, `arxml.v`): a second front end producing the same `Database`, one per CAN cluster, plus what a DBC has no home for (`ArxmlFrame`: TX mode and cadence, receivers, E2E and SecOC layout). `open_database()` (`database.v`) is the ONE way a path becomes a database — `.dbc`, or `.arxml[#Cluster]`, the fragment naming the cluster when the file has several (refused otherwise, naming them) — and it returns the reader's NOTES beside the database; `merge_files_report` carries them plus one line per file it skipped, and the GUI (Log), the headless runner (stderr) and the startup check all print them, because a refusal only the export tool printed was the silent empty database `resolve_asset`'s comment records. `canonical_database_ref` is the one key a loaded database is stored and looked up under (the file's real path, fragment carried along — `real_path` over the whole reference fails on the fragment and echoes its input, which is how two GUI lookups missed in review), and `project.resolve_asset` keeps the fragment out of the file-system question. The parse is cached per file CONTENT (real path + SHA-256; size and mtime miss a same-length rewrite inside one second), because a project opens each channel's databases several times per Start and a real file is seconds and hundreds of MB to parse. Read only; three honesty rules (`ArxmlReport`): a dangling reference is reported by referrer and target, every ignored element kind is counted, and every leaf value the table in `arxml_leaves.v` refuses is said once (#280, below). Per-message extras are keyed by `frame_key(id, ext)` — the identity the rest of candb uses, never the SHORT-NAME, which AUTOSAR scopes per package; E2E and SecOC positions are FRAME-relative (the PDU's own offset in the frame added); a fixed-header profile (4/5/6/7, one OFFSET) and profile 1's ALTERNATING-8-BIT data ids are carried as declared and never exported as a contract they are not; a SecOC freshness or MAC that does not fill whole bytes gets bit positions and a note rather than a wrong byte; a frame mapping several PDUs takes the first PDU's timing and protection and says so; a cluster SHORT-NAME two packages share must be named by path (`cluster_names()`, `--list`); the export carries `VFrameFormat` for FD frames and states FD at bus level in the fragment, because in `ecu.toml` FD is a bus property; and the receive deadline is a placeholder, never a number derived from the cadence, because ComTimeout is not in the file (codex on #273 round 1, eleven findings, all true). Round 2 (seven, all true): an E2E contract is exported only when the CRC and counter SIGNALS start exactly at the declared offsets (a multi-bit application signal that merely contains the offset is not the CRC), the data-id mode is ALL-16-BIT (LOWER-8/12-BIT and ALTERNATING feed the CRC differently) and the profile's checksum is one `e2e_profiles` can compute; a SecOC layout with AUTH-DATA-FRESHNESS is not given byte positions; a one-term rational numerator is a constant (factor 0); once `VFrameFormat` exists every frame states its format; and `Database.messages_from` answers through `senders()`, because an additional transmitter (BO_TX_BU_, or two ECUs OUT on one ARXML frame) simulated nothing while the database listed it. Round 3 (three, all true): the `[[frame]]` fragment and the DBC attributes answer the E2E question through ONE predicate (`e2e_signals`) — round 2 had validated one and left the other on the old rule, the two-places shape this guide warns about; `Signal.receivers` now exists (parsed from the SG_ list, written back sorted, an ARXML frame's receivers on every signal), so the export no longer says `Vector__XXX` about ECUs the file names; and a 1C/SM base type is noted as decoded two's-complement rather than silently signed. Round 4 (five; four as stated, one whose remedy was wrong): the E2E field signals must be EXACTLY the profile's widths (8-bit CRC, 4-bit counter — a narrower signal truncates or wraps); a cluster with several CAN-CLUSTER-CONDITIONAL variants is read from the FIRST (bus and frames alike) and the rest reported, never merged into a bus no variant has; a second frame of one SHORT-NAME at another id is package-qualified (`Other_F`) and said, because a message name is an identity downstream; a sub-millisecond period is never rounded to 0 (that turns a cyclic frame event-driven) and any non-whole millisecond is noted; and the fragment's E2E comment names blobly's primitive as what it is (`crc8_j1850, from PROFILE_01 (blobly's primitive, not the full AUTOSAR profile)`) — the reviewer wanted the export refused until the full profile exists, but the primitive IS what #271 defines the attribute to carry, so the label was fixed and the export kept. Round 5 (four, all true) was the same class a third and fourth time — a package-scoped SHORT-NAME folded into a global identity, now for ECUs and for signals within a message — so `package_qualified` is the ONE rule (`Other_F`, `A_Node`, `Sig2_V`, each said in the report) and `test_package_scoped_names_are_never_folded` covers the class rather than the instance; plus the E2E fields must sit on a byte boundary, because the contract is exported in bytes, and `--dump` prints a fixed-header profile's header offset instead of zero-valued CRC/counter bytes. Round 6 (three, all true): package qualification alone is not injective (`/A_B/F` and `/A/B_F` both spell `A_B_F`; a plain `/C/A_Node` is spelled like a qualified `/A/Node`), so `claim_name` allocates every cluster-, frame-, ECU- and signal-name against the set already given out (`A_Node_2`), deterministic in document order; an enum key is parsed as an integer, not through f64, so 2^53+1 survives; and OPAQUE packing (a byte array) is read as a little-endian integer — the bytes round-trip — with a note that the value is not a quantity. Round 7 (six, all true): the I-PDU-group walk reads DIRECT members only (a deep search filed a contained IN group's PDUs under the parent's OUT direction too, so a receive-only ECU transmitted); an enum key is a u64 (the top half of a 64-bit domain survives); the cluster's collision-free identifier is `ArxmlCluster.bus` (`Body_Body` when two packages share the name), which `cluster()` accepts and the fragment writes as `bus =`; several SW-DATA-DEF-PROPS-CONDITIONAL variants are reported (the first is read — no variation condition is evaluated); a numeric lookup table (COMPU-CONST/V, TAB-NOINTP) is reported rather than filed as an empty enum; and a signal wider than 64 bits is reported and left out rather than decoded into a u64 that cannot hold it. Round 8 (one, true): on a multi-PDU frame a signal's receivers are its OWN PDU's I-PDU-group receivers plus the frame ports' (which listen to the whole frame); the frame keeps the union. Round 9 (five): selectors — cluster `bus` names and ECU names — are allocated in TWO passes (`allocate_names`: every unique SHORT-NAME reserved first, shared ones qualified around them), because one pass handed `A_Bus` to `/A/Bus` while a cluster NAMED A_Bus existed; a constant conversion (one-term numerator) is reported and read as the identity, since factor 0 divides by zero on encode; an update bit and a mixed classic/FD cluster (Message has no per-frame format — the export keeps it) are reported. And ONE finding whose remedy was wrong: a PDU packed MOST-SIGNIFICANT-BYTE-FIRST into its frame is NOT reversed — the PDU-level byte order only says whether START-POSITION counts the first byte's LSB (8k) or MSB (8k+7), so byte k = position/8 under both — verified on a real SystemWeaver export whose only CAN PDU is packed MSB-first, which cantools reads the same way; the skip that finding asked for would have dropped that file's one CAN frame. Round 10 (four, all true): every element name is compared by LOCAL name (`lname`, `descendants` instead of vlib's `get_elements_by_tag`, which compares the prefixed name), so `<ar:AUTOSAR xmlns:ar=…>` reads the same as the default namespace — the fixture is re-read prefixed in the test; TEXTTABLE bounds are compared numerically (`1` and `1.0` are one key); event-controlled timing's repetitions and period are carried on `ArxmlFrame` and named in the fragment, since ecu.toml cannot say a burst; and a description keeps the text inside its inline markup, with the boundary space vlib's parser trims put back. Round 11 (five, all true): the 4.x guard reads the namespace the ROOT is in, prefixed or not; an event burst is a reader note too, not only a fragment comment (the native load path had nothing); a zero-fraction decimal spelling of an enum key is an integer (`9007199254740993.0` never touches f64); an ENUM attribute's default is the quoted choice (`"StandardCAN"`), its values the indices; and attaching a multi-cluster ARXML in the Config panel attaches the FIRST cluster with a notification naming the others and where to change it (`add_dbc`), since the picker hands over a bare path and a bare path would be refused at load with nowhere in the GUI to complete it. Round 12 (three, all true): a signed signal 64 bits wide converts through i64 — `phys_from_raw` sign-extended only BELOW 64, so all-ones read as 1.8e19; fixed in candb, where the scalar lives, not in the reader that accepts the width; a NEGATIVE zero-fraction decimal key is an integer too (`integral_decimal` is the one rule `parse_key` and `parse_i64` both read, and the SIGN is one rule over every literal form — the sign branch had bypassed the exact path and gone through f64, and `-0x1E` read as a float because of its E; the non-negative float fallback goes to u64 directly, since through i64 every key at or above 2^63 was one entry at INT64_MIN); and a declared INIT-VALUE is noted when nonzero, or of any form but a NUMERICAL-VALUE-SPECIFICATION — judged by the DIRECT child, because a deep VALUE search read a TEXT label as 0 and an ARRAY's first element as the whole — because the model has no initial value (a DBC's GenSigStartValue is not read either) and the simulated ECUs start every unconfigured signal at raw 0, a payload that differs from the file; said after the width guard, so nothing claims a simulated outcome for a signal the model does not carry. The self-review of that round also found the 64-bit ENCODE saturating for an unsigned signal (a non-negative rounds through u64 now, a negative through i64 — the add of 1 << length before the mask was a no-op). Round 13 (two, all true): an integral EXPONENT spelling (`9.007199254740993E15`) is the same key as the bare integer — `integral_decimal` moves the point in DIGITS, never through f64, and answers none only when a real fraction remains; and the GUI's script launch builds each channel's database from the LOADED, path-resolved copies (`loaded_dbs_for`, the key `replay_db` uses) rather than re-opening the raw project-relative strings against the process working directory, which handed a script an empty database for a project outside the launch directory while the trace had loaded the same file. Round 14 (four, all true): a description keeps its punctuation tight against inline markup (the boundary space the parser trimmed is put back except before closing punctuation and after an opening bracket, where the source cannot have had one); an exponent is an optionally signed DIGIT string, so `E015` is fifteen; an OPEN or INFINITE constraint bound is read as the closed one the range model keeps and said (`note_open_bounds`), because the excluded value is otherwise advertised as valid; and a compu method giving only COMPU-PHYS-TO-INTERNAL is said and read as identity rather than inverted — one rule for the direction, since a table or a curve has no inverse. Round 15 (seven; six acted on): a TEXTTABLE singleton is two INTEGRAL bounds spelling one integer (`integral_literal`) — `1.1`..`1.9` truncated to one raw key and labelled a range that excludes it; a BITFIELD_TEXTTABLE / MASK method is said and its labels dropped, since a value table keys exact raw values; an encoding that is not a binary integer (BCD-P, BCD-UP, the string encodings) is said like 1C/SM/IEEE754 are; a nonzero UNUSED-BIT-PATTERN is said (the simulated ECUs fill with 0, which a payload checksum also sees); event-only timing is said, because the simulation sends on a cycle and so sends NOTHING for such a frame; and endpoints declared through a PDU-TRIGGERING's I-PDU-PORT-REFS alone are read like frame ports, so a frame with no frame ports still has a sender. Not acted on: inline markup INSIDE a token (`m<E>2</E>`) gets the reconstructed boundary space — vlib's parser trims the boundary, so word-boundary or in-token is a guess either way and the word boundary is the far more common one; recorded as the limit it is. Round 16 (two, both true): an IN I-PDU port listens to ITS PDU only, so on a frame mapping several PDUs those receivers are kept per PDU (by the triggering's I-PDU-REF) and reach only that PDU's signals — the frame keeps the union — where round 15 had put them on every PDU; and a shared method's enum key is filed only if the signal's WIDTH can hold it (a non-negative below the mask, or for a signed signal a sign-extended pattern with the top bit set) — masking every key had folded 255 onto raw 15 of a 4-bit signal and labelled a value the table never named; the rest are dropped and said. Round 17 (two, both true): a SIGNED width holds non-negative keys only below its top bit (200 in an 8-bit signed signal is 0xC8, which decodes as -56); and an E2E contract the reader carries is NOT applied by the native simulation — it protects only what the project's `protect:` entries name, and `dbc.v` does not read the attributes back (#271) — so every protected frame gets a note saying so at load, since a simulated transmitter of it sends an unstamped counter and CRC. Round 18 (five, all true): mixed cyclic-and-event timing is said too (the simulation keeps the cycle only), and so is a cyclic TIME-OFFSET (the phase, which the simulation and the fragment both drop); a SecOC layout is said as carried-not-applied like E2E, with less behind it (no stamping path at all); an enum key's fit is judged by the LITERAL'S SIGN, carried on the scale (`negative`), because at width 64 -2^63 and +2^63 are one pattern; and the export's E2E field predicate takes a byte-aligned field in either byte order (a big-endian CRC byte starts at the field's MSB, `bit + width - 1`). Round 19 (four, all true): a data constraint with SCALE-CONSTRS (sub-intervals) is said, since the range model is one interval; `-0` is zero, not a negative key; a signal the width guard drops no longer claims its name first (the supported same-named signal behind it was renamed for a collision with nothing); and a COMPU-DEFAULT-VALUE is said, since a value table has no default entry. Round 20 (two, both true): a key literal no 64-bit raw key can hold is dropped and said (`key_of`, the fallible parse the filing asks; `parse_key` keeps its 0 fallback for the callers that only compare) where it had been filed on raw zero; and an OPAQUE signal carries NO numeric metadata — a compu method, unit or constraint on its system signal is not applied, or the byte array would be scaled on the way out after all. Round 21 (four, all true): `parse_num` reads a signed hex literal (`-0x1E` was 0 through f64); a linear term of ZERO is the constant conversion in its two-coefficient spelling (factor 0 divided by zero on encode); negative literals are kept in their own table on the cached scale (`neg_values`), because -1 and 2^64-1 are one u64 pattern and a method shared by a signed and an unsigned 64-bit signal may name both; and the provenance comment counts notes, so a partial read travels stamped as one. Round 22 (three, all true): a text-table singleton compares the SIGN with the pattern (`-1`..`18446744073709551615` is a range, not the key -1); `cmd/arxml2dbc` refuses an output path that is the source, or two outputs on one path, before writing anything; and the GUI's script launch takes its database snapshot under `app.mu` (`script_db`), like `replay_db`, since a script started while stopped runs beside an editable Configuration panel whose rebuilds replace `app.dbs`. Round 23 (two, both true): that snapshot cannot be made whole from the reader's side, because `rebuild_from_proj` repopulates `app.dbs` OUTSIDE the lock — so `script_db` reads the channel's files (resolved, through the simulation's loader) and needs no shared array at all, as the headless runner does; and a data constraint with several DATA-CONSTR-RULEs is said, since the first physical (else internal) constraint is read as the range. Round 24 (three, all true): `script_db` loads with the REPORT and puts each note in the Log, so a file gone or broken since the project loaded is named rather than surfacing as "unknown message"; the export's profile mapping is held to `e2e_profiles` (a primitive removed there stops being exported); and a linear scale's own OPEN or INFINITE bound is said like a constraint's, since it becomes the range when nothing overrides it — and (round 25, one, true) said only BY THE SIGNAL THAT USES the domain (`ArxmlScale.domain_open`), because a method whose every user has a data constraint is an exact import, not a partial one. Round 26 (four, all true): a protected frame whose contract the DBC attributes cannot carry (a data-id mode other than ALL-16-BIT, a profile with no primitive) gets a load note saying the export drops it, since a DBC-only consumer never sees the fragment; `split_database_ref` reads a reference ending in `.dbc` as one DBC file, hash and all; `cmd/arxml2dbc` validates `--ecu` before its first write, so a typo cannot leave a regenerated DBC beside a stale fragment; and `--dump` renders a signed signal's choices as the integers they mean, which is what the oracle prints. Round 27 (three, all true): the export's E2E refusal is ONE predicate (`e2e_export_refusal`) that the load note asks too, so a layout refusal (a CRC offset inside an application signal) is said like a mode refusal; a description in several languages keeps the first and says so; and an ECU legitimately named `Vector__XXX` — the DBC's no-transmitter placeholder, which `senders()` normalises away — is exported under a collision-free name with a note. Round 28 (three, all true): several I-PDU-TIMING specifications on one PDU are said (the first is read); a `J-1939-CLUSTER` is in the ignored census, since a whole bus discarded is not a complete read; and the per-content cache coalesces concurrent misses (`arxml_cache_inflight`), so a project rebuild overlapping a script reload parses a hundreds-of-MB file once. Round 29 (three, all true): a reference ending in `.arxml` is one file too (a cluster fragment never ends in a database extension); a TRANSMISSION-MODE-FALSE-TIMING beside the TRUE one is said, since the mode condition is not evaluated; and a FAILED parse is cached under the same path-and-content key, so a broken file attached to a project is not reparsed on every rebuild while repaired bytes retry. Round 30 (two, both true, one class): frame and signal names are allocated over the WHOLE cluster / message before any is given out (`allocate_names_first_keeps`) — every unique SHORT-NAME reserved first, then the first of a shared one keeps it and the rest are qualified — where one pass in document order handed `B_F` to the duplicate `/B/F` and renamed the frame whose own name is B_F. Round 31 (two, both true): that allocation runs over the DISTINCT signal paths, so one signal mapped twice is `V` and `V_2` rather than both qualified; and `raw_from_phys` clamps to the domain's endpoints before the cast, because f64 rounds 2^64-1 and 2^63-1 up to the next power of two, which cast to 0 and to the sign bit — the opposite endpoint. Round 32 (three, all true): the occurrence-level claim for a frame triggered under two ids, or a signal mapped twice, is seeded with EVERY allocated name, so a repeat's suffix cannot take a name reserved for a frame or signal not yet reached (`/A/F` twice, then `/B/F_2`); and rational coefficients with no numerator are said rather than indexed. Round 33 (four, all true): `cmd/arxml2dbc` compares output paths by parent-real-path plus base name (a leaf that does not exist yet has no real path) and STAGES both artifacts before moving either into place; a LENGTH missing or 0 is dropped and said (a negative shift otherwise); and `project.resolve_asset` tries the COMPLETE path before splitting an ARXML fragment, since it resolves recordings and manifests too. Round 34 (four, all true): a negative enum key must lie in the signed width's INTEGER range (a pattern test let -257 label raw -1 of an 8-bit signal); an integral exponent past 20 digits is refused before it is expanded (`1E100000000` built a hundred million zeroes); a frame with no FRAME-LENGTH or one outside 0..64 is not read, since a simulated sender sizes its payload from it; and `cmd/arxml2dbc` stages under a per-invocation temporary name. Round 35 (six, all true): a classic frame longer than 8 bytes, a missing / negative / too-wide IDENTIFIER, a negative START-POSITION and a zero linear denominator are each not read and said (the three backends would otherwise carry, clamp and refuse one file three ways, a missing id would claim the real id-0 frame, a negative bit would index below the payload); the exponent bound counts normalised digits, not the point (`0.1E20` fits); and `cmd/arxml2dbc` publishes a pair under a directory lock on its destination, since two exports' moves could interleave — every FILE destination locked, in sorted order, since two exports sharing only the fragment each held their own DBC's lock (round 36); and an IDENTIFIER is parsed fallibly (`key_of`), so `invalid` is refused rather than read as id 0. Round 37 (four, all true): `frame_admission` is the ONE predicate the naming pre-pass and the import pass share (a rejected triggering had reserved a name the imported frame then lost); a START-POSITION is parsed fallibly; a signal whose bit traversal leaves the payload is not read (`signal_fits_frame`, both byte orders); and `cmd/arxml2dbc` releases its locks on every exit path, since `exit` runs no defer. Round 38 (four, all true): FRAME-LENGTH and a PDU's START-POSITION are parsed fallibly too (a malformed length claimed an id as a zero-length frame; a negative PDU position was rounded to byte 0 before any guard saw it); `integral_literal` checks hex DIGITS (`0xZZ` passed and read as 0); and an E2E export needs exactly one DATA-ID, since an absent id is not the id 0. Round 39 (six, all true): an E2E DATA-ID or offset that is not an integer marks the contract `malformed` and refuses the export (0 is a legitimate id and offset); FRAME-LENGTH goes through the bounded parse (`0x10000000000000000` read as 0); the DBC writer declares in BU_ every node a BO_ or SG_ line references; and `cmd/arxml2dbc` resolves an EXISTING leaf whole (a symlinked input compared by base name), and sets each destination aside before its replacement moves in, so a second move failing puts the first back. Round 40 (nine, all true, and a CLASS): every field that means a count, a width, an offset or an id is read by ONE fallible bounded reader (`int_text` / `int_in` / `int_of`) — missing, not an integer, overflowing or out of range is refused and said — where the lenient parsers had handed back 0, a legitimate value in every one of those fields, so five rounds had each found another instance; a CAN-FD length must be one a DLC expresses; CAN-ADDRESSING-MODE must be STANDARD or EXTENDED; a TIME-PERIOD that is not a number is said, as is a cyclic timing left with no cycle; a mapping with no PDU-REF is said; and `cmd/arxml2dbc`'s rollback removes a destination it created. Round 41 (seven, all true): one sign at most in an integer literal (`+-256` read as 256); NUMBER-OF-REPETITIONS goes through the reporting reader; PACKING-BYTE-ORDER must be one of its three values (a typo read as little-endian); a signal is bounded by its PDU's LENGTH as well as the frame's; an unreadable authentic LENGTH means no SecOC layout; a rational coefficient that is not a number drops the scale rather than inventing an offset; and a SECURED-I-PDU with no PAYLOAD-REF is said. Round 42 (seven, all true): `num_text` is the fallible reader for every NUMBER (constraint bounds, scale domains, coefficients, periods) as `int_text` is for integers, and it takes the hex parse_num always took; a PDU whose LENGTH cannot be read maps no signals; COMMUNICATION-DIRECTION must be OUT or IN; a SecOC layout must fit the secured PDU; an E2E field must lie inside the protected DATA-LENGTH; and `cmd/arxml2dbc` prints its stdout artifact only after the file artifacts are published. Round 43 (seven, all true; one fixed, six filed as #280): `cmd/arxml2dbc` refuses a DIRECTORY as a destination (publication would have renamed it aside and put a file in its place). The other six — unrecognised `CAN-FRAME-TX/RX-BEHAVIOR`, unvalidated PDU-group direction, `1e`/`1e+` numerals, SecOC bounded by the frame as well as the PDU, a zero E2E DATA-LENGTH, an unreported missing I-SIGNAL-I-PDU-REF — are the tail of the malformed-input class that rounds 31–42 closed two large parts of (integers, numbers), deferred as ONE issue with the shape of the next class fix (`enum_of`) written down, so the review can end on what the reader does with real files. Round 44 (three, all true): `cmd/arxml2dbc` publishes to the CANONICAL path the locks name, so a destination that is a symlink keeps the link and the file lands at its target (a link to nothing is refused); the DBC writer normalises a signal's receivers the way `parse_sg` reads them — no empties, no placeholder beside a real name, no repeats — so a programmatic Signal round-trips as a fixpoint; and a COMPU-SCALE bound that is present and not a number is said rather than silently leaving the signal at [0|0], the one site the round-42 number reader had not reached. Round 45 (four, all true; one fixed, three to #280): `cmd/arxml2dbc` creates its staging file EXCLUSIVELY under an unpredictable name (`O_CREAT|O_EXCL`, which refuses a path that already exists as anything, a symlink included), because `os.write_file` follows a link somebody else planted at a guessable name in a shared directory and truncates whatever it points at; the other three — a PDU longer than its frame admitted on its start byte, `1e309` read as infinity, a signal mapping with no reference dropped without a note — are the malformed-input tail, on #280. Round 46 (two, both true, both fixed): `BO_TX_BU_` is written as `apply_tx_bu` reads it (no empty, no repeat), the receivers rule of round 44 applied to the other node list; and `cmd/arxml2dbc` announces `wrote` only once every destination is published, since a later move failing rolls the earlier one back. Round 47 (two, both true, both fixed): a regenerated file keeps the mode of the one it replaces (a 0600 database stayed 0600; new files take the 0644 default), and a publication that rolls back removes its staging files too. Round 48: clean. Round 49 (two, both true; one fixed, one to #280): an IN I-PDU port whose triggering names a PDU this file cannot resolve is NOT promoted to a receiver of the whole frame — the port listened to that PDU, the dereference already recorded it as unresolved, and widening it claimed every other PDU's signals for that ECU; an unreadable NUMERICAL-VALUE-SPECIFICATION is the malformed-input tail, on #280. Round 50 (two, both true, both fixed): deleting a node in the DBC editor is `candb.Database.remove_node` — the list AND every sender, transmitter and receiver naming it — because the writer now declares every referenced node and a list-only deletion came straight back on Save; and `cmd/arxml2dbc` flushes and checks the close of its staging file before it is a candidate for publication. Round 51 (one, true, fixed): a mapping with no PACKING-BYTE-ORDER is not read as little-endian — a multi-bit signal's start position and byte significance both depend on the order, so there is nothing to read it by; said, and a one-bit signal reads the same either way. Round 52 (three, all true; one fixed, two to #280): `cmd/arxml2dbc` resolves each destination ONCE and validates, locks, stages and publishes that one path, so a symlink retargeted between the check and the write cannot aim the publication at the source; an overflowing hex literal read as 0 and two scales with one key and two labels are the malformed-input tail, on #280. Round 53 (one, true, fixed): a destination that exists and is not a regular file (a FIFO, a socket, `/dev/null`) is refused — publication would have renamed it aside, replaced it with a file and deleted it at cleanup. Round 54 (two, both true; one fixed, one to #280): the regular-file check is asked again at the moment of replacement, which narrows the window to the rename itself — rename-based publication in a directory another account can write to has no atomic form, and the code says so; an overflowing UNUSED-BIT-PATTERN read as 0 is the malformed-input tail, on #280. Round 55 (one, true, fixed): the bad-coefficient guard used the coefficient's text as its sentinel, so an EMPTY <V/> read as "none bad" and became a 0; a flag beside the text. **Then #280, the class fix those rounds were asking for: `arxml_leaves.v`**, a table of every LEAF element the reader depends on and what it may hold (integer with bounds, finite number, one of an enumeration), checked ONCE over the whole tree before extraction — every element the table names, whether or not a frame leads to it — and the ONE source of bounds for `int_in`/`int_of`/`num_of`, so a limit is stated once and a field read without a row is reported as the reader's defect. A refused value is one note (`<path>: <PARENT>/<TAG> "<text>" <why>; not read`) and the extractor then skips the field in silence; the relations the table cannot state (a PDU past its frame — which bounds the SecOC layout it carries — a STANDARD id past 11 bits, TX and RX behaviours disagreeing, a mapping with no reference, two scales on one raw key) are rules beside the readers, and a MISSING element is the extractor's to say, since the table sees only what is present. Bounds are bus-agnostic where the element is (the pass walks the whole file, and a gateway's description carries Ethernet PDUs beside the CAN cluster); the CAN limits are those relations. The declared schema (`xsi:schemaLocation`) is recorded in the report and the provenance CM_ (`schema=AUTOSAR_4-2-2`), and a name outside the 4.x family is said. One table-driven test feeds every row a bad value; that is the class covered once, which fifteen rounds of one site at a time never delivered. The reader is an EXTRACTOR into the existing model, deliberately not a model of ARXML. `dbc/example.arxml` is the hand-written 4.2 fixture, one feature per frame; **cantools is the oracle** (`sut/arxml_oracle.py` diffs against `cmd/arxml2dbc --dump`; the four known differences are in its docstring). `cmd/arxml2dbc` is the export for the two consumers that cannot read ARXML — blobly_emb's build and a user who needs to edit: a DBC carrying #271's E2E attributes (`E2ECounterSignal`/`E2ECrcSignal`/`E2EProfile`/`E2EDataId`) and a provenance `CM_` (source, SHA-256, reader version, cluster, dropped / unresolved / note counts — a partial read is stamped as one) — both emitted by the writer itself (`to_dbc_with`, `DbcExtras`), not spliced into its text — plus a `[[frame]]` fragment for `ecu.toml`. `candb.e2e_profiles` is the ONE list of checksum primitives; sim's three validation sites and the profile mapper read it. The ARXML is the source of truth and blobly_net reads it natively; an ARXML-backed database is READ-ONLY in the DBC editor, because Save serialises DBC text to the selected path. `dbc.v` does not yet read the attributes back — that is #271 |
| `isotp` | ISO-TP (ISO 15765-2) transport. Two backends behind one `Channel` (`isotp.v`): the kernel CAN_ISOTP socket on Linux, and `software.v` — the state machine in V — EVERYWHERE ELSE, which is what flash, the shell, scripts and the GUI's diagnostics use against real ECUs on Windows. **The send side honours Flow Control** (#226, `flowcontrol.v`): CTS/WAIT/OVERFLOW, block size and STmin, decoded once by `parse_flow_control` and paced by `send`. It read the PCI nibble alone until then — every `0x3x` meant carry on, at full rate — which is invisible in-process, where nothing paces or blocks, and is exactly what a bootloader answers by dropping the transfer; flash's data blocks are that path. The reserved STmin ranges fall back to 0x7F, the SLOWEST legal value, because ISO says so and because a code the sender does not understand must not be answered by transmitting flat out. STmin is measured FROM THE LAST FRAME (monotonic ns — its 100..900 us scale is unmeasurable on a millisecond tick) and not counted within a block: at BS=1 every frame is the first of its block, so pacing 'between frames in a block' paces nothing, and ISO exempts no boundary. An ORPHAN Flow Control is skipped where a First or Single Frame is awaited, like a stale Consecutive Frame is: this side never expects one, and the new aborts leave the peer's later FCs queued — read as a message they surfaced as `unexpected PCI 0x31` in place of the next reply. And the usual next move after a failed send is another SEND, which that skip does not cover, so a channel whose segmented send aborted is marked `fc_dirty` and drains before its next First Frame — before it, so nothing dropped can be an answer to it; a flag rather than an unconditional drain, which would also discard a reply the caller has not read. **The mark is cleared by that drain OR by a `recv` that SUCCEEDS** (#296) — on the success returns, not where the frame is recognised, so a recv that then fails on an invalid Single Frame or a reassembly timeout keeps the window: a message that got through means the orphans ahead of it were skipped, so the abort is no longer the most recent thing on the channel, and a later segmented send must not drain away a reply the caller has since been sent. Cleared there rather than at the next send of any shape, because a drain in front of a Single Frame would block for a quiet window and discard queued rx on a path that is neither — on `uds.Server.serve`, a client's retransmitted request. And when that skip is what produced a silence, the timeout SAYS so (`orphan_note`): skipping an orphan FC is right, but a bare `timeout` is the one answer that says nothing about what is on the wire. EVERY pacing delay is rounded UP to a whole millisecond (`pacing_sleep_us`): V's Windows `sleep` is `C.Sleep(int(duration / millisecond))`, which truncates at any magnitude — the 100..900 us scale is `Sleep(0)`, and since the value passed is a REMAINDER (STmin less the time already waited) a 30 ms separation asks for 29,900 us and sleeps 29. Unconditionally, not `$if windows`: the Linux cost is under a millisecond per frame on a path `open` does not take there, and a pure function with a platform branch is one whose test can only be right on one platform. The dirty-channel drain is a QUIET WINDOW (`flush_rx`, which until then had no caller), because the peer may be mid-burst when the abort happens and the last frame has not arrived yet — and it is a mitigation, not a proof: an ISO-TP Flow Control carries no transfer identity, so one arriving after the window cannot be told from this transfer's own. WAIT re-arms the wait, so it is COUNTED (`n_wft_max`): a peer that only ever waits never trips a timeout, since a frame keeps arriving. What this channel sends AS A RECEIVER is still the permissive answer (CTS/0/0) — asking for no pacing is the one thing a receiver may always do, and `recv`'s own deadline bounds the read side. The receive side matches `(id, extended)` and rejects RTR, which the kernel socket gets from `CAN_EFF_FLAG` and this one had to be told (#225 r18–19) |
...
| `wiretap` | whose frame is this? — the record of what we put on the wire, matched against what comes back, so the trace's `origin` column can separate our tester (`TX`) and our simulation (`TX-S`) from the real ECU (`RX`). **Eviction is IN PLACE** (`victim_index` + `delete`; `expire`/`drop_expired` drop their aged prefix with `delete_many`): at a full ring every `note()` used to rebuild the array to make room for ONE record — a map of indices, three passes and a fresh 1024-entry copy, ~150 KB of garbage per emitted frame, measured at ~1 GB/s inside the GUI at replay rates and the cause of a once-a-second GC stutter that left a 13-bus replay 11 s behind (#299). The SIBLING path had the same shape at the rates the cap never reaches: below ~500 frames/s records AGE out instead, and both `expire` (every emitted frame) and `drop_expired` (every received one) copied the whole remaining ring to drop a prefix, ~400 KB a step, under `app.mu` — found by the self-review of the first fix, which is what that review is for. Same priority (settled → verdictless → oldest), same verdicts, and `reportable()` is the ONE spelling of which eviction is a verdict, for both paths. `bench_test.v` pins both classes IN BYTES, because allocation is deterministic where time is not: 356 per note against 180,996, 256 per aged step against 422,160, each failing on the old code. Measured from inside the real GUI by `cmd/blobly_net/probe.v` (`BLOBLY_PROBE_LOG=<file>`, inert otherwise), which starts the run through the `BLOBLY_AUTOSTART` gate on the GUI thread — a run started from a worker thread inside the GL settle window is the crash that gate exists for — and scores each frame against `player.due_at_ms`, the ONE spelling of the schedule (the GUI had a second, scaled by speed and clamped to the pass). The rest of the rate went in #300 — 146 → 28.5 MB/s, found by the probe's per-section attribution (`sec_*` in its summary; the blocking sections' means are upper bounds) and by switch-off runs, not by reading: the identity predicates answered in place, snapshots into buffers the GUI thread keeps, the grouped view's caches, the inproc fan-out through `try_push`, the closure per send built once, one bus label per MF4 bus rather than per record — and the worker's PACING: on Windows a sub-millisecond `time.sleep` is `Sleep(0)`, so it had spun between frames, 80 M ticks in a 70 s run, taking `app.mu` on each; `modules/player/pace_windows.v` sleeps whole milliseconds and waits out the last on the stopwatch and asks for the 1 ms period (`pace_nix.v` is one accurate nanosleep) (70% → 99% of frames within 1 ms). The residual is the collection's LENGTH: the live set after a collection is 454 MB from a 17 MB file, because the loader builds 1.23 M entries each pointing at its own payload. That is the arena (#303, `canlog` row); what remains after it is streaming, `docs/streaming_replay.md` |
| `sim` | simulated ECUs — tests need no hardware; `doip_entity.v` decides what a DoIP channel hosts and `doip_host.v` is the served-side handler, both shared by the GUI and the headless runner. **`verify:` describes the ECU UNDER TEST**, and two checks say so when a project forgets it: `validate_verify` reports entries that resolve to nothing, and `verify_covers`/`self_sent_warning` (#95) report a FRAME OF OURS carrying a message `verify:` lists — from the send's SUCCESS path (beside `count_tx_load`), once per message per wire per run (the latch is `App.verify_said`, not the VerifySet, because a set is rebuilt per rx_loop and a reader handoff would repeat the line). Matched through `sim.Coverage` — an INDEX built once at Start (three maps: the keys `verify:` names, every message the wire's databases define, and PGN -> key, the last built ONLY from frames the file DECLARED J1939), so the question costs three map lookups on a path that runs at every successful send. It walked the databases instead until codex measured what that means on the transmit path: O(frames x messages) under the global mutex, for traffic unrelated to the verified message. It recognises a live J1939 id — priority and source address the DBC does not carry — is recognised, and RETURNS the MESSAGE's key, never the wire's, so the notice latches once however many source addresses replay it (`resolve`'s adoption deliberately does NOT mark the adopted key: it bought nothing the PGN walk did not already do, and made one entry latch under two keys). ONLY for keys that came from `verify:`: a node's `protect:` entry describes a message that node SENDS, so our own frames carrying it are the normal case, and `VerifySet.from_verify` records the difference the merge would otherwise lose. The RX path has refused to verify our own frames since the trace learned provenance (`!ours` in `rx_loop`, #92), so such an entry cannot report a false failure; it goes quiet instead, checking only what somebody else sends and nothing at all if we are the only sender, and that silence is what gets reported. **Asked of the WIRE, not of the project**: predicting it from the configuration was wrong four ways at once (a DBC with no `GenMsgCycleTime` is never transmitted, a cyclic generator transmits and is equally ours, two simulated senders each read as the other's real one, and entries `validate_verify` had already declared ignored got a second contradictory warning). A frame we are putting on the wire needs no model of any of that. Asked at the EMIT and not on the RX path, which is where it was first written: PCAN does not echo our sends (`transport.echoes_own_sends` is false for it), so our own frames never come back and an RX-side test could never fire on the backend it matters most for (codex on #289). Two more placements of the same notice fell to review: `note_emit` runs BEFORE the driver takes the frame, so a refused send (listen-only, a rejected frame, a queue that stays full) warned about a frame that never went out AND consumed the once-per-run latch the real one would need; and publishing the coverage from `rx_loop` tied it to the MONITOR opening, so a fast sender — or a Lua tool outliving Stop — could transmit while the map was still empty and be missed for good. `start()` publishes the coverage while the transmit mutexes are still HELD — released first, a guardless tap kept by a Lua tool could send into the previous run's coverage and latch, and only the PROVENANCE half (which keys `verify:` describes, and under what name): the verifiers carry counter state `rx_loop` mutates, and V's maps are references. A PGN entry is built from the `VerifyOrigin` the entry RESOLVED TO — name, id, format and declaration travelling together, so a duplicate definition in a sibling database (the merge keeps the first) cannot lend a declaration the verified message does not have — and it is dropped where two verified messages share a PGN, since neither name is more right than the other and this notice's contract is that it names the one `verify:` listed. A PGN entry needs `Message.j1939` — `BA_ "VFrameFormat" ... J1939PG`, parsed back by `dbc.v` for this (#289) — because a 29-bit id alone is not evidence: `j1939_pgn` computes a PGN for any extended value, so a UDS request and its response share one, and a response this app hosts on an id defined nowhere was being reported as the request the project asked to verify. A database with no such attribute (most of them) gets exact-key matching only: a missing advisory, never a wrong one. It reports the FRAME and not a property of the project: a single Quick Send, or one `can.send` from a script, is equally ours and says nothing about how the project is configured — the same refusal to judge that `stale.v` makes about silence. **The VALUE SOURCE is shared with the generators** (#269): what a signal emits (`const`/`sine`/`sawtooth`/`counter`/`stepmod`, `project.wave_kinds`) is orthogonal to WHO emits it, so a generator signal carries the same `GenCfg` in `SenderSig.wave` and is evaluated by the same `sim.gen_from_cfg` — one parser (`parse_gencfg`), one writer (`gen_params_inline`), one evaluator. `TX` vs `TX-S` stays the identity distinction the trace column and restbus semantics lean on. `wave.typ == ''` means no source: send the static `value`, which is also the fallback when a source cannot be evaluated (`gen_source_invalid` — an unknown type or a zero divisor — reported by `generator_source_warnings` at Start and as the parameter is typed). The picker offers `value` in place of `const`, because for a generator a constant IS its static value. The per-send sources ride the generator's send index, the simulator's per-message `send_n` equivalent; time-based ones are evaluated from the RUN epoch, one clock sample per frame. A project that uses a generator source is written as schema **v3** (`version_for`), everything else stays v2 and openable by older builds |
| `player` | replay a recording at its recorded cadence — the GUI's **Replay panel** is its transport face (pause/seek/speed via `set_speed`, position-preserving; `set_repeat` arms looping, a PASSIVE flag read only at the end of a pass, so it never acts when you set it). **The panel/worker command protocol is `control.v`**, here rather than in the GUI so a test can reach it: a TARGET state rather than a toggle, a tri-state loop sentinel (a bool has no value spare for "no request", so loop could never stay on), `take()` reading and clearing together, and the ordering `apply_latched` -> `status` -> `apply_timed` -- loop must be applied BEFORE the status that acknowledges it, or the panel's latch clears on a value the worker has not applied yet. Two of #160's four findings landed in this path and the second was a defect of the first one's fix, because `cmd/blobly_net` has no tests and CI runs `v test modules/` only (#161). And decide what NOT to replay — `restbus.v` subtracts the ECU under test by DBC sender so a capture can drive a rest bus without the SUT arguing with a recording of itself; `multibus.v` maps SEVERAL recorded buses onto several live ones from ONE clock, because the SUT gateways between them and the timing across buses is what it polices. `due_into` is `due()` into the caller's buffer, for the worker that asks a thousand times a second, and `due_at_ms` the ONE spelling of when an entry plays (#299, #300) |
| `canlog`, `mf4` | `candump -l` files; native ASAM MDF4 (`.mf4`) reader. **A loaded recording is the ARENA** (`canlog.Log`, #303): pointer-free 80-byte `Row`s in one block V allocates no-scan, and a label table the rows name by index — a million rows measured at 76 MB with five full collections over them taking 1 ms in all, where a `[]LogEntry` was 80 bytes plus a cloned payload plus a label string per frame, millions of objects the collector marked on every pass and the LENGTH of every replay pause (the frequency went in #299/#300). Entries come out as VIEWS (`at`, `frame`: the payload aliases the row — `copy_at` for a consumer that keeps one, `entries()` for the old shape, tests and small callers), `from_entries` is the one conversion the other way, and `relabelled` is the same rows under another label table, which is how a replay plan maps recorded buses onto live ones without copying a frame. The loader fills it directly (`parse_cg` writes rows, the payload copied in rather than cloned beside; the sort is an index sort and one permutation, the last copy the recording ever takes) and the player plays a selection of it (`Player{log, sel}`, `new_player_log`; `MultiPlan{log, sel}` from `build_multi_log`; `sel_on_bus`/`subtract`/`census_sel` are the one body their entry-taking twins wrap). Measured on the 13-bus replay: live set after a collection 454 → 257 MB, collections in 70 s 10 → 4, 99.4% of frames within 1 ms. What is left of the live set is a second copy the Replay panel's census makes. Tens-of-GB recordings want a bounded WINDOW of these rows filled from disk — `docs/streaming_replay.md` is that design; the arena is its cell |
...
| `project` | `.blobnet` project files. **A generator's `bus:` is a channel NAME** (#97, `senderbus.v`), because an interface cannot say which CHANNEL a generator belongs to: two configured channels may share one wire deliberately, so `bus: inproc:CAN2` picks the wire and not the owner — which is why picking the second channel of a shared wire reverted on the next save/reload, the GUI having grown a runtime owner (`SenderRT.chan`) with nowhere in the file to put it. `resolve_sender_bus` is the ONE place that answers the key, and it answers with a VERDICT rather than a string, because neither form is an identity on its own: an interface string is still understood — that is the migration, since `docs/simulation.md` told people to write one, and it is the only way to name a wire that is **not a configured channel at all** (`pcan:…@250000`, which Start opens a tap for on purpose) — and channel NAMES are no more enforced-unique than interfaces, so `bus:` can name two channels just as it could name two rows on one wire. A name is tried first, so a channel named like a wire is the channel; a value two channels answer to is `.ambiguous`, which still transmits where the wire is not in doubt (today's behaviour) and is said once per generator at Start by `sender_bus_warnings`, never once per sharer — the GUI's runtime rows attribute an unowned generator to every channel on its wire, so a line keyed by channel would repeat. A project using the name form declares schema **v4**, since a build released before #97 hands the value to the transport as a device name. `destination_conflicts()` is the ONE place a project is refused for asking a wire to be two things — one mode, one rate, one protocol (a `canfd` row and a `can` row on one **Vector, Kvaser, CANsub or PCAN** wire is refused from the file, not as a channel that fails halfway through a Start — those four are the backends that configure a data phase, and `transport.adapter_configures_data_phase` is the one place to say so. It is NOT every CAN backend: SocketCAN and the software buses answer `false`, because there the data phase is the link's or nobody's, so a mixture there is a warning and not a refusal), and (#167) one physical channel: two Vector *application* channels assigned to one *physical* channel are two wires here and one to the driver, so the comparison is pure and testable (`alias_conflicts`) while the resolution behind it (`transport.physical_wire_key`) is per-platform and answers `none` wherever no driver can say |
...
it;
and `cmd/blobly_net/txhealth/` and `cmd/blobly_net/txclaim/`, the transmit-only wire census
and reader ownership (#142). Each such wire gets its own receive handle, drained continuously:
SocketCAN error frames and Vector chip-state replies advance health inside `recv`. The reader
...
(no window or hardware required; put `modules|libs|cmd/blobly_net` first in `-path` so V's
test-mode lookup cannot select `cmd/script` instead of the engine module);
...
**And the operator can no longer change the run's TOPOLOGY under it** (#120): the Buses panel's
enable tick decides which buses a measurement OPENS, and doing that mid-run meant spawning readers
against ports other threads hold, handing wires between owners, and re-deciding a wire's mode after
its ports were configured. That handler was 142 lines when #120 proposed removing it and **340** by
the time it was — #114, #143, #159, #164, #165, #167 and #199 had each added another guard to the
same path, which is the argument stated as a measurement. It refuses now, naming the row; changing
topology is Stop, tick, Start, as the Replay panel's tick already required. What stays live is the
WORKLOAD — simulated ECUs, fault injection, Quick Send, generators, pause, trace filters — which
mutates what the workers read without creating or destroying them. **The runtime `enabled` bit still
moves mid-run**, though, so nothing that reads it got simpler: `rx_loop` disables a wire's rows when
its adapter fails (`workers.v`), which is what `dest_left_the_run_locked` is about and why the
reader-handoff machinery stays.
...
- [can_hardware.md](docs/can_hardware.md) · [windows_can_hardware.md](docs/windows_can_hardware.md) ·
  ...
  [releasing.md](docs/releasing.md) · [streaming_replay.md](docs/streaming_replay.md) (replaying a
  recording that does not fit in memory — the design; the arena is its cell)
