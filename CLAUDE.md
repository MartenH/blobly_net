...
| `transport` (load) | **Bus load** (`busload.v`): bit-times a frame occupies on the wire — classic 47/67 + 8n with worst-case stuffing stated as the choice, CAN-FD with its data phase scaled by the rate switch — over the wire's nominal rate. The Buses row shows the last 60 s as a strip and this second as a number; the toolbar carries the busiest running wire. Ours and theirs count alike (a frame on the wire is load whoever sent it) |
