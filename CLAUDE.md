...
| `transport` (load) | **Bus load** (`busload.v`): bit-times a frame occupies on the wire — classic 47/67 + 8n with worst-case stuffing stated as the choice ((len-1)/4, the textbook 135 bits for a standard 8-byte frame), a remote frame at its true empty length, CAN-FD with its data phase scaled by the rate switch — over the wire's nominal rate. The Buses row shows the last 60 s as a strip and this second as a number; the toolbar carries the busiest running wire. Ours and theirs count alike (a frame on the wire is load whoever sent it) — ours once the driver ACCEPTED the frame, so a refusal has nothing to refund, and onto the wire's RUNNING row, so an alias row's sends are the wire's load and not a skipped row's, and carried to the successor when the reader moves like health and cadence are. **Of DECODED frames only**, and labelled so: an error frame, a retransmission and an overrun drop occupied the wire and arrive as health or as a count, never with a length, so in an error storm the gauge under-reads and the fault ladder beside it is what says so |
...
`cmd/blobly_net/saverule/`, `cmd/blobly_net/logfile/` and `cmd/blobly_net/taprule/`, the
GUI-local rules with tests — what Save means (#250); where the session log lives, what it is
called, how many are kept and what its header says (#258); and the transmit-tap lifecycle —
when a tap is filed, when a sender is ready, when a manual send may fall back, when an
abandoned tap is dropped — the path #257 took ten review rounds on (#260) — kept in GUI-free
sub-modules so they run without ImGui), `scripts/runtests.sh`,
