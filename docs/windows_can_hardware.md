...
| Own sends do not come back as RX | ✅ one handle per channel | ✅ local TX echo turned off per handle (2026-08-29; canlib's default delivers one handle's writes to every other handle on the channel, and the app opens several per wire — each cyclic frame showed twice, TX then RX 0.5 ms later) | ✅ the shim drops the transmitting port's confirmation; the other ports' copies are claimed by `wiretap` | ✅ the echo says it is one, and is claimed |
